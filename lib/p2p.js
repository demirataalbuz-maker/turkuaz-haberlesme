...
const { EventEmitter } = require('events') // Bare'de bare-events'e eşlenir (package.json imports)

const NS = 'turkuaz/v1'

// sha256 — sodium ile (Node VE Bare'de aynı; node:crypto Bare'de yok).
// Eski createHash/update zinciriyle bayt bayt aynı sonucu verir → topic'ler uyumlu.
function topicOf (...parts) {
  const data = Buffer.concat([Buffer.from(NS), ...parts.map(p => Buffer.from(p))])
...
class P2P extends EventEmitter {
  ...
  // Arkadas kodu Noise/DHT public key'inin kendisi. Topic lookup hedef henuz
  // ilan vermeden biterse uzun sure bos kalabilir; explicit peer ise hedef
  // sonradan acilsa bile Hyperswarm tarafindan yeniden denenir.
  joinPeer (peerHex) {
    if (typeof peerHex !== 'string' || !/^[0-9a-f]{64}$/.test(peerHex)) return
    this.swarm.joinPeer(Buffer.from(peerHex, 'hex'))
  }
