...
function createCore ({ store, bootstrap, iceServers, version = '', log = () => {}, exit = () => {} } = {}) {
  ...
  for (const r of rooms) if (!r.voiceChannels) r.voiceChannels = ['genel']
  ...
  p2p.on('hello', (peer, info) => {
    ...
    if (f) {
      ...
      f.avatar = info.avatar ? sanitizeAvatar(info.avatar) : f.avatar
    ...
    switch (msg.t) {
      case 'friend-request': {
        ...
        if (f) {
          ...
          requests.push({ code: peer, name: String(msg.name || '').slice(0, 64), avatar: sanitizeAvatar(msg.avatar), ts: Date.now() })
      ...
      case 'dm': {
        ...
        if (dre) rec.re = dre
        const dpv = sanitizePrev(msg.prev)
        if (dpv) rec.prev = dpv
        const dfw = sanitizeFw(msg.fw)
        if (dfw) rec.fw = dfw
      ...
      case 'room': {
        ...
        // Canlıda kimlik zaten bağlantıdan doğrulanmış; imza geçmiş aktarımı
        // için saklanır. Tutmayan imza kaydedilmez (geçmişte zaten elenirdi).
        if (roomVerify(msgParts(msg.room, rec), msg.sig, peer)) rec.sig = msg.sig
        const rre = sanitizeRe(msg.re)
        if (rre) rec.re = rre
        const rpv = sanitizePrev(msg.prev)
        if (rpv) rec.prev = rpv
        const rfw = sanitizeFw(msg.fw)
        if (rfw) rec.fw = rfw
        store.appendMessage(conv, rec)
        seenSet(conv).add(msg.id)
        emitMsg(conv, foldOne(conv, rec))
      ...
      case 'room-log': {
        ...
        const conv = 'room-' + msg.room
        if (seenSet(conv).has(ev.evId)) return
        // Geçersiz imzayı kayda geçirme; olay canlıda yine işlenir
        const cleanForSig = cleanEvent(ev)
        if (!cleanForSig || !roomVerify(msgParts(msg.room, cleanForSig), ev.sig, peer)) delete ev.sig
        appendEvent(conv, ev)
        break
      }
      case 'room-file': {
      ...
      case 'hist-res': {
        ...
        for (const l of msg.lines.slice(0, 1000)) {
          ...
          // Geçmiş satırları YAZARIN imzasını taşımak zorunda: aktaran üye
          // 'from' alanına başkasını yazamaz, içeriği değiştiremez.
          // (v0.5.1 öncesi imzasız satırlar yeni üyelere aktarılmaz.)
          if (!roomVerify(msgParts(msg.room, l), l.sig, l.from)) continue
          // re/prev/fw imza kapsamı dışında (canlıda da öyle) — aktaran üye
          // kurcalayabilir; en azından biçim ve boyutlarını sıkıca sınırla.
          const hre = sanitizeRe(l.re); if (hre) l.re = hre; else delete l.re
          const hpv = sanitizePrev(l.prev); if (hpv) l.prev = hpv; else delete l.prev
          const hfw = sanitizeFw(l.fw); if (hfw) l.fw = hfw; else delete l.fw
      ...
      case 'seen-upto': {
        // Karşı taraf DM'i görüntüledi (okundu bilgisi ✓✓). ts: gördüğü en yeni
        // mesajın zamanı. Geriye gitmez (>= koruması) — eski/duplike paket zararsız.
        const f = friendOf(peer)
        const ts = Number(msg.ts)
        if (!f || !Number.isFinite(ts)) return
        if (!(f.seenUpTo >= ts)) {
          f.seenUpTo = ts
          store.saveFriends(friends)
          broadcast({ t: 'seen', conv: 'dm-' + peer, upTo: ts })
        }
        break
      }
      case 'rtc': {
      ...
      case 'room-channel': {
        // Yeni metin kanalını anında öğren (boş kanal da yayılsın — mesaj beklemeden)
        if (typeof msg.room !== 'string' || typeof msg.ch !== 'string') return
        const r = roomOf(msg.room)
        if (!r || !p2p.peerInRoom(peer, msg.room) || isBanned(r, peer)) return
        learnChannel(r, normCh(msg.ch))
        break
      }
      case 'room-voicechannel': {
        if (typeof msg.room !== 'string' || typeof msg.ch !== 'string') return
        const r = roomOf(msg.room)
        if (!r || !p2p.peerInRoom(peer, msg.room) || isBanned(r, peer)) return
        learnVoiceChannel(r, normCh(msg.ch))
        break
      }
      case 'ack': {
        const q = outbox[peer]
  ...
  // Avatar: emoji (≤8) VEYA küçük raster resim data-URL'i (png/jpeg/webp, ≤20KB).
  // SVG/başka şema KABUL EDİLMEZ — uzak avatar <img src>'e gittiği için script riski.
  function sanitizeAvatar (a) {
    a = String(a || '')
    if (/^data:image\/(png|jpeg|webp);base64,[A-Za-z0-9+/=]+$/.test(a) && a.length <= 20000) return a
    return a.slice(0, 8)
  }
  ...
  // Link önizlemesi: gönderenin ürettiği kart. re gibi imza kapsamı DIŞINDA
  // taşınır; alıcıda biçim/boyut sıkıca sınırlanır (url http(s), görsel yalnız
  // küçük data-URI — alıcı hiçbir dış adrese bağlanmaz).
  const PREV_IMG_RE = /^data:image\/(?:jpeg|png|webp);base64,[A-Za-z0-9+/=]+$/
  function sanitizePrev (p) {
    if (!p || typeof p !== 'object' || typeof p.url !== 'string') return undefined
    if (!/^https?:\/\//i.test(p.url) || p.url.length > 2048) return undefined
    const out = {
      url: p.url,
      title: String(p.title || '').slice(0, 200),
      desc: String(p.desc || '').slice(0, 300),
      site: String(p.site || '').slice(0, 100)
    }
    if (!out.title && !out.desc) return undefined
    if (typeof p.img === 'string' && p.img.length <= 80000 && PREV_IMG_RE.test(p.img)) out.img = p.img
    return out
  }

  // İletilen mesaj etiketi ("aslı: <yazar>") — re/prev ile aynı statüde.
  function sanitizeFw (fw) {
    if (!fw || typeof fw !== 'object') return undefined
    const name = String(fw.name || '').slice(0, 64)
    return name ? { name } : undefined
  }

  // ---- oda mesajı imzaları ----
  // Canlı mesajlarda kimlik bağlantının Noise anahtarıdır (taklit edilemez).
  // Ama GEÇMİŞ (hist-res) başka bir üyeden aktarılır: imza olmasa aktaran,
  // satırların 'from' alanına istediğini yazabilirdi. Bu yüzden her oda
  // mesajı/dosyası/olayı yazarın anahtarıyla imzalanır; geçmişte imzasız ya da
  // imzası tutmayan satırlar reddedilir. (DM'lerde geçmiş aktarımı olmadığı
  // için DM imzasızdır.)
  const SIG_RE = /^[0-9a-f]{128}$/
  function roomSig (parts) {
    return hcrypto.sign(Buffer.from(parts.join('\n')), p2p.keyPair.secretKey).toString('hex')
  }
  function roomVerify (parts, sig, from) {
    if (typeof sig !== 'string' || !SIG_RE.test(sig) || !/^[0-9a-f]{64}$/.test(String(from || ''))) return false
    try {
      return hcrypto.verify(Buffer.from(parts.join('\n')), Buffer.from(sig, 'hex'), Buffer.from(from, 'hex'))
    } catch { return false }
  }
  // İmzalanan alanlar: kayıt biçimindeki (temizlenmiş) değerler. Gönderen de
  // alıcı da aynı temizlikten geçirdiği için bayt bayt aynı payload çıkar.
  function msgParts (room, l) {
    const name = String(l.name || 'anon')
    if (l.ev) return ['ev', room, String(l.evId || ''), String(l.ev), String(l.id || ''), String(l.ts), name, String(l.text || ''), String(l.emoji || '')]
    if (l.file) return ['file', room, String(l.id || ''), String(l.ch || 'genel'), String(l.ts), name, String(l.file.fid || ''), String(l.file.fname || ''), String(l.file.mime || ''), String(l.file.size || 0)]
    return ['msg', room, String(l.id || ''), String(l.ch || 'genel'), String(l.ts), name, String(l.text || '')]
  }

  function learnChannel (r, ch) {
    if (!r.channels.includes(ch)) { r.channels.push(ch); store.saveRooms(rooms); pushState() }
  }
  function learnVoiceChannel (r, ch) {
    if (!r.voiceChannels) r.voiceChannels = ['genel']
    if (!r.voiceChannels.includes(ch)) { r.voiceChannels.push(ch); store.saveRooms(rooms); pushState() }
  }
  ...
  function cleanEvent (ev) {
    ...
    if (!['react', 'edit', 'del', 'pin'].includes(clean.ev)) return null
    return clean
  }
  ...
  function appendEvent (conv, ev) {
    ...
    if (!clean) return
    if (typeof ev.sig === 'string' && SIG_RE.test(ev.sig)) clean.sig = ev.sig
  ...
  function sendDM (code, text, re, prev, fw) {
    ...
    seenSet(conv).add(id)
    emitMsg(conv, foldOne(conv, rec))
    queueDM(code, { t: 'dm', id, ack: id, text, ts: rec.ts, re, prev, fw })
    pushState()
  }
  ...
  function sendFile (target, fname, mime, dataB64) {
    ...
    if (target.code) { // DM
      ...
      rec.sig = roomSig(msgParts(target.room, rec))
      store.appendMessage(conv, rec)
      seenSet(conv).add(id)
      emitMsg(conv, foldOne(conv, rec))
      for (const peerHex of p2p.roomPeers(target.room)) {
        ...
        p2p.sendToPeer(peerHex, { t: 'room-file', room: target.room, id, fid, fname, mime, size: buf.length, ch, name: identity.name, ts, sig: rec.sig })
      }
    }
    pushState()
  }
  ...
  function sendEvent (conv, ev) { // react / edit / del / pin
    ...
    if (conv.startsWith('dm-')) {
      ...
      const clean = cleanEvent(ev)
      if (!clean) return
      ev.sig = roomSig(msgParts(topic, clean))
      appendEvent(conv, ev)
  ...
  function sendRoom (topic, ch, text, re, prev, fw) {
    const r = roomOf(topic)
    text = String(text || '').slice(0, 4000)
    ...
    rec.sig = roomSig(msgParts(topic, rec))
    if (re) rec.re = re
    if (prev) rec.prev = prev
    if (fw) rec.fw = fw
    store.appendMessage(conv, rec)
    seenSet(conv).add(id)
    emitMsg(conv, foldOne(conv, rec))
    p2p.broadcastRoom(topic, { t: 'room', room: topic, id, name: identity.name, text, ch, ts: rec.ts, re, prev, fw, sig: rec.sig })
  }

  // Mesajı başka sohbete ilet: kaynak yerel geçmişten bulunur, yeni bir mesaj
  // olarak (fw = "aslı: <yazar>" etiketiyle) gönderilir. Dosyalar için blob
  // zaten diskte — yeniden yükleme yok, aynı fid ile taşınır.
  function forwardMsg (target, srcConv, msgId) {
    if (typeof srcConv !== 'string' || !/^(dm|room)-[0-9a-f]+$/.test(srcConv)) return
    const src = store.loadFolded(srcConv, 10000).find(x => x.id === msgId)
    if (!src || src.deleted || src.call) return
    const fw = { name: src.name || 'anon' }
    if (src.file) {
      const fid = src.file.fid
      const meta = filesIdx[fid]
      if (!meta) return
      const id = uuid()
      const ts = Date.now()
      const fileMeta = { fid, fname: meta.fname, mime: meta.mime, size: meta.size }
      if (target.code) {
        const f = friendOf(target.code)
        if (!f) return
        const conv = 'dm-' + target.code
        const rec = { id, from: myCode, name: identity.name, text: '', file: fileMeta, fw, ts }
        store.appendMessage(conv, rec)
        seenSet(conv).add(id)
        emitMsg(conv, foldOne(conv, rec))
        queueDM(target.code, { t: 'dm-file', id, ack: id, fid, fname: meta.fname, mime: meta.mime, ts, fw })
      } else if (target.room) {
        const r = roomOf(target.room)
        if (!r) return
        const ch = normCh(target.ch)
        const conv = 'room-' + target.room
        const rec = { id, from: myCode, name: identity.name, text: '', ch, file: fileMeta, fw, ts }
        rec.sig = roomSig(msgParts(target.room, rec))
        store.appendMessage(conv, rec)
        seenSet(conv).add(id)
        emitMsg(conv, foldOne(conv, rec))
        for (const peerHex of p2p.roomPeers(target.room)) {
          if (isBanned(r, peerHex)) continue
          sendFileChunks(peerHex, fid)
          p2p.sendToPeer(peerHex, { t: 'room-file', room: target.room, id, fid, fname: meta.fname, mime: meta.mime, size: meta.size, ch, name: identity.name, ts, fw, sig: rec.sig })
        }
      }
      pushState()
    } else if (src.text) {
      if (target.code) sendDM(target.code, src.text, undefined, src.prev, fw)
      else if (target.room) sendRoom(target.room, target.ch, src.text, undefined, src.prev, fw)
    }
  }
  ...
  function stateObj () {
    return {
      ...
      friends: friends.map(f => ({
        ...
        status: f.status, online: p2p.isOnline(f.code), seenUpTo: f.seenUpTo || 0
      })),
      requests,
      rooms: rooms.map(r => ({
        ...
        channels: r.channels, voiceChannels: r.voiceChannels || ['genel'], banned: r.mod ? r.mod.banned : [],
  ...
  function handleUI (m, reply = broadcast) {
    switch (m.t) {
      ...
      case 'set-profile':
        ...
        if (m.avatar !== undefined) identity.avatar = sanitizeAvatar(m.avatar)
      ...
      case 'send-dm': sendDM(m.code, m.text, m.re, m.prev); break
      case 'send-room': sendRoom(m.topic, m.ch, m.text, m.re, m.prev); break
      case 'forward':
        if (typeof m.msgId === 'string') {
          forwardMsg(m.code ? { code: m.code } : { room: m.room, ch: m.ch }, m.conv, m.msgId)
        }
        break
      case 'send-file':
      ...
      case 'typing':
        ...
        break
      case 'mark-seen': {
        // Kullanıcı DM'i görüntüledi → karşıya görüldü bilgisi (✓✓) gönder.
        // Anlık olaydır, outbox'a girmez: karşı taraf offline'sa düşer (typing gibi).
        // sentSeenUpTo: aynı bilgiyi tekrar tekrar yollamamak için yerel not.
        const f = typeof m.code === 'string' ? friendOf(m.code) : null
        const ts = Number(m.upTo)
        if (!f || !Number.isFinite(ts)) break
        if (p2p.isOnline(m.code) && !(f.sentSeenUpTo >= ts)) {
          f.sentSeenUpTo = ts
          store.saveFriends(friends)
          p2p.sendToPeer(m.code, { t: 'seen-upto', ts })
        }
        break
      }
      ...
      case 'add-channel': {
        ...
        if (r) {
          learnChannel(r, normCh(m.ch))
          p2p.broadcastRoom(m.room, { t: 'room-channel', room: m.room, ch: normCh(m.ch) }) // anında yay
        }
        break
      }
      case 'add-voice-channel': {
        const r = roomOf(m.room)
        if (r) {
          learnVoiceChannel(r, normCh(m.ch))
          p2p.broadcastRoom(m.room, { t: 'room-voicechannel', room: m.room, ch: normCh(m.ch) })
        }
        break
      }
      case 'rtc':
