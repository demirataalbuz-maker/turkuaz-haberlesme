...
// Bir ses kanalındaki katılımcılar (vch verilirse ona göre süz). Sen seste olmasan
// da room-ev "görülenler" ile bilinir; her görülen kişi hangi kanaldaysa oraya düşer.
function voiceParticipants (topic, vch) {
  const out = new Map()
  const V = window.Voice
  if (!V) return out
  const seen = V.seen && V.seen.get(topic)
  if (seen) for (const [code, info] of seen) {
    if (vch && (info.vch || 'genel') !== vch) continue
    out.set(code, { code, name: nameForCode(code, info.name) })
  }
  if (V.room === topic && (!vch || (V._vch || 'genel') === vch)) {
...
function renderSidebarRoom (r, el) {
  ...
  // ---- SES KANALLARI (çoklu — her biri ayrı ses alanı) ----
  ...
  const vAdd = document.createElement('button'); vAdd.className = 'ch-add-mini'; vAdd.textContent = '+'
  vAdd.title = 'Yeni ses kanalı'; vAdd.setAttribute('aria-label', 'Yeni ses kanalı ekle')
  vAdd.onclick = async () => {
    const ch = await askText('Yeni ses kanalı', '', { placeholder: 'kanal-adı', okLabel: 'Oluştur', maxlen: 24 })
    if (ch) send({ t: 'add-voice-channel', room: r.topic, ch })
  }
  vgt.appendChild(vAdd); vg.appendChild(vgt)
  const vchans = (r.voiceChannels && r.voiceChannels.length) ? r.voiceChannels : ['genel']
  for (const vch of vchans) {
    const parts = voiceParticipants(r.topic, vch)
    const here = !!(window.Voice && Voice.room === r.topic && (Voice._vch || 'genel') === vch)
    const vrow = document.createElement('div')
    vrow.className = 'vc-row' + (here ? ' joined' : '')
    vrow.setAttribute('role', 'button'); vrow.setAttribute('tabindex', '0')
    vrow.setAttribute('aria-label', 'Sesli sohbet — ' + vch + (here ? ' (içindesin)' : ''))
    const ic = document.createElement('span'); ic.className = 'vc-ic'; ic.textContent = '🔊'
    const vl = document.createElement('span'); vl.className = 'ch-label'; vl.textContent = vch
    vrow.append(ic, vl)
    if (parts.size) { const c = document.createElement('span'); c.className = 'vc-count'; c.textContent = parts.size; vrow.appendChild(c) }
    const joinVoice = () => { if (window.Voice && !here) Voice.join(vch) }
